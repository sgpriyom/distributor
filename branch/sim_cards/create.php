...
$page_title = 'Add SIM Card Transaction';
$db = new Database();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $auto_quantity = isset($_POST['auto_quantity']) ? intval($_POST['auto_quantity']) : 0;
    $total_sold = isset($_POST['total_sold']) ? intval($_POST['total_sold']) : 0;

    if (empty($transaction_date) || $quantity_received < 0 || $opening_stock < 0 || $auto_quantity < 0 || $total_sold < 0) {
    ...
    // Calculations
    $total_available = $quantity_received + $opening_stock + $auto_quantity;
    ...
    try {
        $stmt = $conn->prepare("
            ...
            (branch_id, transaction_date, quantity_received, opening_stock, auto_quantity, total_available, total_sold, closing_stock, created_at) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())
        ");
        $stmt->execute([$branch_id, $transaction_date, $quantity_received, $opening_stock, $auto_quantity, $total_available, $total_sold, $closing_stock]);
    ...
    <div class="container-fluid py-4">
        ...
        <div class="card mt-4">
            <div class="card-header">
                <h5 class="card-title mb-0">Add SIM Card Transaction</h5>
            </div>
            <div class="card-body">
                <form action="" method="POST">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            <label class="form-label">Auto Quantity</label>
                            <input type="number" class="form-control" name="auto_quantity" min="0" required>
                        </div>
                        <div class="col-md-6">
